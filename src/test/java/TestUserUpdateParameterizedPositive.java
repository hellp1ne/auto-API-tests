...
public class TestUserUpdateParameterizedPositive {
    ...
    public TestUserUpdateParameterizedPositive(UserUpdateRequest userUpdateRequest) {
    ...
    @Test
    @DisplayName("Test updating user data with authorization")
    @Description("Verify that a user can update their data when authorized")
    public void testUserUpdateWithAuthorization() {
        // Send a request to update user data with authorization
        Response response = userApiClient.updateUser(userUpdateRequest, this.accessToken);
